...
class ResNet18(nn.Module):
    def __init__(self, num_classes=131):  # num_classes for ImageNet is 1000
...
# MAGIC %md
# MAGIC ##### The SIPaKMeD database is publicly available and it can be used for experimental purposes with the request to cite the following paper:
# MAGIC ##### Marina E. Plissiti, Panagiotis Dimitrakopoulos, Giorgos Sfikas, Christophoros Nikou, Olga Krikoni, Antonia Charchanti, SIPAKMED: A new dataset for feature and image based classification of normal and pathological cervical cells in Pap smear images, IEEE International Conference on Image Processing (ICIP) 2018, Athens, Greece, 7-10 October 2018. 
# MAGIC
# MAGIC The SIPaKMeD Database consists of 4049 images of isolated cells that have been manually cropped from 966 cluster cell images of Pap smear slides. These images were acquired through a CCD camera adapted to an optical microscope. The cell images are divided into five categories containing normal, abnormal and benign cells.

# COMMAND ----------

import os

HF_DATASETS_CACHE = "/Volumes/will_smith/datasets/sipakmed"
...
mri_scans = load_dataset('Simezu/brain-tumour-MRI-scan')
...
labels = set(medical_images['train']['txt'])
...
train_test = medical_images['train'].train_test_split(test_size=0.2)
...
df = label_encoder.fit_transform(test_df['txt'])
...
train_dataset =  Dataset.from_dict({"image": train_df, "label": subset_imagenet_train['label']}).with_format("torch", device=device)
test_dataset =  Dataset.from_dict({"image": tes_df, "label": subset_imagenet_val['label']}).with_format("torch", device=device)
