...
labels = set(image_net["train"]["label"])
num_class = len(labels)
num_class

# COMMAND ----------

print(f"Dataset size: {image_net['train'].info.size_in_bytes / (1024 ** 3):.2f} gb")
...
class ResNet50(nn.Module):
    def __init__(self, num_classes=1000):  # num_classes for cifar is 10 
...
def train_one_epoch(model, device, data_loader, optimizer, epoch):
    ...
    for batch in data_loader:
      ...
      print(f"Batch: {total} Running loss: {running_loss:.4f} Accuracy: {correct}/{total}")
...
def train(log_dir):
  ...
  print(f"Starting training using {NUM_WORKERS} workers with {NUM_GPUS_PER_NODE} GPUs")
  ...
  train_dataset =  Dataset.from_dict({"image": image_net['train']['image'], "label": image_net["train"]["label"]}).with_format("torch", device=device)
...
def test(log_dir):
  ...
  print(f"Starting testing using {NUM_WORKERS} workers with {NUM_GPUS_PER_NODE} GPUs")
  ...
  test_dataset =  Dataset.from_dict({"image": image_net['test']['image'], "label": image_net["test"]["label"]}).with_format("torch", device=device)
...
def main_fn(directory):
  ...
  # Apply transformations directly to the dataset
  train_dataset =  Dataset.from_dict({"image": image_net['train']['image'], "label": image_net["train"]["label"]}).with_format("torch", device="cuda")
  ...
  if global_rank == 0:
    ...
    test_dataset =  Dataset.from_dict({"image": image_net['test']['image'], "label": image_net["test"]["label"]}).with_format("torch", device="cuda")
...
output = TorchDistributor(num_processes=4, local_mode=True, use_gpu=True).run(main_fn, single_node_multi_gpu_dir)
